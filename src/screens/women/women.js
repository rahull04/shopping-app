...
import img1 from '../../components/images/kurta.png';
import Footer from '../../components/footer/footer';
import HeaderImg from './../images/women-heading.jpg';
...
export default class Women extends React.Component{
      ...
      //  History.push('');
        ...
        if( slugURLval != -1 ){ // URL has item reference
          ...
          if (temp.some(card => card.slug === slugVal)) { // URL has CORRECT item reference
            ...
            History.push('/catalog/women'); // URL has INCORRECT item reference
            console.log('incor item')
          }
        }else{
          History.push('/catalog/women'); // URL doesnt have item reference
  ...
  render() {
    ...
    return (
      <div>
        <Header />
        <h1 className='category-title'>{this.props.name}</h1>   
